$(function() {
	...
	function selectOne(firstId,secondId) {
		$.ajax({
			type : "get",
			url : "/RolesPermission/" + firstId+"/"+secondId,
			success : function(data) {
				if (data.code != 200) {
					alert(data.message);
					return;
				}
				updVar.firstId = data.data.roles.rolesId;
				updVar.secondId = data.data.permission.permisId;
				updVar.first = data.data.roles;
				updVar.second = data.data.permission;
				optionFlag = 2;
				$("#updModal").modal();
			}
		});
	...
	function firstSelectSub(){
		if(optionFlag == 1){
			...
			updVar.first = null;
			updVar.first = {};
			updVar.first.rolesId = con.value;
			updVar.first.rolesName = con.container.parent().nextAll().eq(1).html();
		}
	...
	function secondSelectSub(){
		if(optionFlag == 1){
			...
			updVar.second = null;
			updVar.second = {};
			updVar.second.permisId = con.value;
			updVar.second.permisUrl = con.container.parent().nextAll().eq(2).html();
